...
pub async fn generate_query_engine_compose(
    ...
    let queryengine_dir = format!("{}/asap-query-engine", code_dir);
    let template_path = format!("{}/query-engine-rust-cli-compose.yml.j2", queryengine_dir);
    let helper_script = format!("{}/asap-tools/experiments/generate_queryengine_compose.py", code_dir);

    // Compose file output path (inside ProjectASAP)
    let compose_output_path = format!("{}/asap-tools/docker/generated_compose_files/query-engine-compose.yml", code_dir);

    // Ensure compose output directory exists
    tokio::fs::create_dir_all(format!("{}/asap-tools/docker/generated_compose_files", code_dir)).await?;
...
pub async fn generate_controller_compose(
    ...
    // Paths for template and script
    let controller_dir = format!("{}/asap-planner-rs", code_dir);
    let template_path = format!("{}/controller-cli-compose.yml.j2", controller_dir);
    let helper_script = format!("{}/asap-tools/experiments/generate_controller_compose.py", code_dir);

    // Compose file output path (inside ProjectASAP)
    let compose_output_path = format!("{}/asap-tools/docker/generated_compose_files/controller-compose.yml", code_dir);
...
async fn call_generate_queryengine_compose_script(
    ...
    let project_root = get_project_root()?;
    let uv_project = format!("{}/asap-tools/asap-cli/uv_configs/generate_compose", project_root.to_string_lossy());
...
pub async fn generate_fake_exporters_compose(
    ...
    let fake_exporter_dir = match language {
        "python" => format!("{}/asap-tools/prometheus-exporters/fake_exporter/fake_exporter_python", code_dir),
        "rust" => format!("{}/asap-tools/prometheus-exporters/fake_exporter/fake_exporter_rust/fake_exporter", code_dir),
    ...
    let helper_script = format!("{}/asap-tools/experiments/generate_fake_exporter_compose.py", code_dir);

    // Ensure compose output directories exist
    tokio::fs::create_dir_all(format!("{}/asap-tools/docker/generated_compose_files", code_dir)).await?;
    tokio::fs::create_dir_all(format!("{}/asap-tools/docker/generated_compose_files/fake_exporter_composes", code_dir)).await?;
    ...
    for i in 0..*num_ports_per_server {
        ...
        let compose_output_path = format!("{}/asap-tools/docker/generated_compose_files/fake_exporter_composes/{}", code_dir, compose_name);
...
async fn generate_master_fake_exporters_compose(
    ...
    let project_root = get_project_root()?;
    let master_compose_path = format!("{}/asap-tools/docker/generated_compose_files/fake-exporter-compose.yml", project_root.to_string_lossy());
    ...
    for (idx, compose_file) in compose_files.iter().enumerate() {
        let compose_path = format!("{}/asap-tools/docker/generated_compose_files/fake_exporter_composes/{}", project_root.to_string_lossy(), compose_file);
...
pub async fn asap_up() -> Result<(), Box<dyn std::error::Error>> {
    let project_root = get_project_root()?;
    let compose_path = format!("{}/asap-tools/docker/asap-docker-compose.yml", project_root.to_string_lossy());
