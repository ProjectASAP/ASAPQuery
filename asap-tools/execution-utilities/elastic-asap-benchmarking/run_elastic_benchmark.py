...
def run_benchmark(
    ...
    with open(output_csv, "w", newline="") as csvfile:
        ...
        for query_id, sql in queries:
            ...
            if error:
                print(f"✗ {error}")
                writer.writerow([query_id, pattern, f"{latency_ms:.2f}", 0, "", error, mode])
            else:
...
def check_connection(elastic_host: str, elastic_port: int, api_key: Optional[str], mode: str = "baseline") -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✗ Elasticsearch SQL returned HTTP {response.status_code}: {response.text[:200]}")
            return False
    except Exception as e:
