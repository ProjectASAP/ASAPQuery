...
Usage:
    ...
    # Run the benchmark on Elastic and ASAP
    python run_benchmark.py \
        --mode both \
...
def run_query(
    ...
    timeout: int = 30,
    debug: bool = False,
    database: str = "clickhouse",  # "clickhouse" | "elastic"
    ...
    try:
        ...
        print(response.json())
        # Change return type to (latency_ms, result_text, num_rows, error)
        # In run_query, replace the success return:

        if response.status_code == 200:
            if database == "elastic":
                data = response.json()

                # Handle native hits response (/_search or ASAP proxy)
                if "hits" in data:
                    hits = data["hits"].get("hits", [])
                    if hits:
                        # Derive column names from the first doc's _source keys
                        col_names = list(hits[0].get("_source", {}).keys())
                        formatted_rows = []
                        for hit in hits:
                            source = hit.get("_source", {})
                            formatted_rows.append(", ".join(
                                f"{k}={source.get(k)}" for k in col_names
                            ))
                        result_text = "\n".join(formatted_rows)
                        num_rows = len(hits)
                    else:
                ...
                # Handle SQL API response (/_sql?format=json)
                elif "rows" in data:
                    rows = data.get("rows", [])
            ...
            return latency_ms, None, 0, f"HTTP {response.status_code}: {response.text[:200]}"

    # Also update the error/timeout returns:
    ...
    with open(output_csv, "w", newline="") as csvfile:
        ...
        for query_id, sql in queries:
            ...
            if last_error:
                ...
                # For elastic, row count is the number of \n-delimited lines (one per formatted row)
                # For clickhouse, same applies since each result row is on its own line
                num_rows = last_row_count
...
def main():
    ...
    parser.add_argument("--elastic-host", default=None,
        help="Elasticsearch host — if provided, runs elastic benchmark using --baseline-sql-file")
    parser.add_argument("--elastic-port", type=int, default=DEFAULT_ELASTIC_PORT)
    parser.add_argument("--elastic-api-key", default=None)
    ...
    if args.mode in ("baseline", "both"):
        if args.elastic_host:
            run_benchmark(
                ...
                endpoint_url=f"http://{args.elastic_host}:{args.elastic_port}/_sql?format=json",
    ...
    if args.mode in ("asap", "both"):
        asap_endpoint = (
            f"http://localhost:8088/_sql?format=json"
            if args.elastic_host
            else args.asap_url
        )
        run_benchmark(
            sql_file=Path(args.asap_sql_file),
            endpoint_url=asap_endpoint,
            output_csv=asap_csv,
            mode="asap",
            database="elastic" if args.elastic_host else "clickhouse",
            api_key=args.elastic_api_key,
            query_filter=query_filter,
            timeout=args.timeout,
            repeat=args.repeat,
