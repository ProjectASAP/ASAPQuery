...
def prepare_h2o_json(input_path: str, output_path: str, max_rows: int = 0):
...
def prepare_h2o_parquet(input_path: str, output_path: str, max_rows: int = 0):
    """Convert H2O CSV to Arroyo-compatible Parquet with synthetic timestamps."""
    try:
        import pandas as pd
        import pyarrow as pa
        import pyarrow.parquet as pq
    except ImportError:
        print("Error: pandas and pyarrow are required for parquet output.")
    ...
    print(f"  Read {len(df):,} rows. Building timestamps...")
...
def prepare_h2o(input_path: str, output_path: str, max_rows: int = 0, fmt: str = "json"):
    if fmt == "parquet":
        prepare_h2o_parquet(input_path, output_path, max_rows)
    else:
        prepare_h2o_json(input_path, output_path, max_rows)
...
def main():
    ...
    parser.add_argument(
        ...
        default="json",
        help="Output format (default: json). Parquet only supported for h2o.",
    )
    parser.add_argument("--input", required=True, help="Path to raw input file")
    parser.add_argument("--output", required=True, help="Path to write prepared output file")
    parser.add_argument(
        "--max-rows",
        type=int,
        default=0,
    ...
    if args.format == "parquet" and args.dataset != "h2o":
        parser.error("--format parquet is only supported for --dataset h2o")
    ...
    if args.dataset == "clickbench":
        ...
        prepare_h2o(args.input, args.output, args.max_rows, fmt=args.format)
