...
SAMPLE_SIZE = 5_000_000  # rows to read for timestamp auto-detection
...
def generate_sql_files(
    ...
    order_by_clause = ", ".join(group_by_columns)
    ...
    for i, end_ts in enumerate(window_ends):
        ...
        es_lines.append(
            ...
            f"WHERE {es_where} GROUP BY {group_by_clause} ORDER BY {order_by_clause};"
        )
