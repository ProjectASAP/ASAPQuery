...
def load_h2o_elasticsearch(
    ...
    total = 0
    errors = 0
    for ok, info in helpers.streaming_bulk(es, generate_docs(), chunk_size=batch_size, raise_on_error=False):
        if ok:
            total += 1
        else:
            errors += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Load a dataset into ClickHouse / Elastic for baseline comparison",
    ...
    parser.add_argument("--es-index", default="h2o_benchmark")
    parser.add_argument("--es-api-key", default=None)
    parser.add_argument("--es-bulk-size", type=int, default=5000)
