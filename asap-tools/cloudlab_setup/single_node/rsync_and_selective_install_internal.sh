...
if [ ${#SYNCED_COMPONENTS[@]} -eq 0 ] && [ ${#SYNCED_FILES[@]} -eq 0 ]; then
    ...
    # Filter out asap-tools component as it doesn't have an installation directory
    INSTALLABLE_COMPONENTS=()
    for COMPONENT in "${SYNCED_COMPONENTS[@]}"; do
        if [ "$COMPONENT" != "asap-tools" ]; then
    ...
    if [ ${#INSTALLABLE_COMPONENTS[@]} -gt 0 ]; then
        ...
        ssh $OPTIONS $USERNAME@$HOSTNAME "cd $DESTINATION_DIR/asap-tools/installation && ./only_install_internal_components.sh ${INSTALLABLE_COMPONENTS[*]}"
