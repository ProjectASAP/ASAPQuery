#!/usr/bin/env python3
"""
Benchmark baseline ClickHouse vs ASAP query engine for MAX(pkt_len) queries.

Supports:
- Window presets: 1s, 5s, 1m, 5m
...
def build_query(
    ...
    base = (
        "SELECT dstip, MAX(pkt_len) AS max_pkt_len\n"
        f"FROM {table}\n"
        f"WHERE {where}\n"
        "GROUP BY dstip\n"
    )
    ...
    if query_type == "basic":
        ...
        suffix = "ORDER BY max_pkt_len DESC\nFORMAT TabSeparated"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark MAX(dstip) query across windows and variants."
    )
    parser.add_argument("--runs", type=int, default=DEFAULT_RUNS)
    parser.add_argument("--warmup", type=int, default=2)
    parser.add_argument(
        "--window",
        choices=["all", "1s", "5s", "1m", "5m"],
        default="all",
    ...
    windows = ["1s", "5s", "1m", "5m"] if args.window == "all" else [args.window]
    variants = (
    ...
    for window_name in windows:
        for query_type in variants:
            run_variant(window_name, query_type, runs, warmup, args.fixed_end)
