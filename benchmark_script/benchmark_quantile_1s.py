#!/usr/bin/env python3
"""
Benchmark baseline ClickHouse vs ASAP query engine for quantile SQL variants:

1) basic:
   SELECT ... quantile(0.99)(pkt_len) ... GROUP BY proto, srcip, dstip
2) orderby:
   SELECT ... GROUP BY ... ORDER BY p99_pkt_len DESC
3) orderby_limit:
   SELECT ... GROUP BY ... ORDER BY p99_pkt_len DESC LIMIT 10
"""
...
def build_query(table: str, fixed_end: str | None, query_type: str) -> str:
    ...
    if query_type == "basic":
        ...
        suffix = "ORDER BY p99_pkt_len DESC\nFORMAT TabSeparated"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark quantile query for 1s tumbling window."
    )
