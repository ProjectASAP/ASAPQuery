...
use serde_json;
use crate::ast_parsing::query_info::{ElasticDSLQuery, Predicate, GroupBySpec, AggregationType, FieldName, TermValue};
...
pub fn walk_ast_and_extract_info(ast: &dsl::Search) -> Option<ElasticDSLQuery> {
    ...
    let (target_field, aggregation_type, group_by_spec) = walk_aggregations_and_extract_info(&ast.aggs)?;
    Some(ElasticDSLQuery::new(target_field, predicates, group_by_spec, aggregation_type))
}
    ...
    for query in filters {
        match query {
            dsl::Query::Term(term_query) => {
                ...
                predicates.push(Predicate::Term { field, value: term_value });
            }
            ...
            dsl::Query::Range(range_query) => {
                ...
                let gte_value = gte.as_ref().and_then(|gte_term| map_term_to_json_value(gte_term));
                let lte_value = lte.as_ref().and_then(|lte_term| map_term_to_json_value(lte_term));
                predicates.push(Predicate::Range { field, gte: gte_value, lte: lte_value });
            }
            _ => {
                // Handle other query types
                todo!()
...
pub fn walk_aggregations_and_extract_info(aggregations: &dsl::Aggregations) -> Option<(FieldName, AggregationType, Option<GroupBySpec>)> {
    ...
    for (_, agg) in aggregations {
        match agg {
            dsl::Aggregation::MultiTerms(terms_agg) => {
                ...
                let field_names: Vec<String> = terms_agg.multi_terms.terms.iter().filter_map(|multi_term| multi_term.field.clone()).collect();
                let field_names: Vec<String> = field_names.iter().map(|s| strip_keyword_suffix(s).to_owned()).collect();
                ...
                let (target_field, aggregation_type) = find_aggregation_info(&terms_agg.aggs.clone())?;
...
pub fn extract_aggregation_info(agg: &dsl::Aggregation) -> Option<(FieldName, AggregationType)> {
    ...
    match agg {
        ...
        dsl::Aggregation::Percentiles(percentiles_agg) => {
            ...
            let percents = percentiles_agg.percentiles.percents.clone().unwrap_or_default();
...
fn map_term_to_json_value(term: &dsl::Term) -> Option<TermValue> {
    ...
    match term {
        ...
        dsl::Term::Float32(value) => {
            Some(TermValue::Float(value.clone() as f64))
        },
        dsl::Term::Float64(value) => {
            Some(TermValue::Float(value.clone()))
        },
...
mod tests {
        ...
        fn walk_aggregations_and_extract_info_extracts_multi_terms_and_percentiles() {
                ...
                assert_eq!(
                        agg_type,
                        AggregationType::Percentiles(vec![50.0, 95.0])
                );
                assert_eq!(
                        group_by,
